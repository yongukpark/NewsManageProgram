...
{
	newsList = ndb.selectFS(first, second);
}
ndb.close();
%>
<title></title>
...
<body>
	...
	<footer id="page_number">
		<ul>
			...
			%>
			<li onclick="location.href='categoryNews.jsp?pageNum=<%=i%>&first=<%=request.getParameter("first")%>&second=<%=request.getParameter("second")%>'"><%=i%></li>
			<%
			}
			}
