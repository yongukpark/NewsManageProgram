...
String s = request.getParameter("search_input");
NewsDB ndb = new NewsDB();
ArrayList<News> newsList = new ArrayList<News>();
if(request.getParameter("condition").equals("기자 이름"))
{
	newsList = ndb.selectReporterName(s);
}
else if(request.getParameter("condition").equals("기사 제목"))
{
	newsList = ndb.selectTitle(s);
}
else if(request.getParameter("condition").equals("기사 내용"))
{
	newsList = ndb.selectContent(s);
}
ndb.close();
%>
<title></title>
...
<body>
	...
	<article>
		<div id="current_category"><%=request.getParameter("condition") %>에 "<%=request.getParameter("search_input")%>"이 포함된 검색 결과 입니다.</div>
		<ul id="list">
	...
	<footer id="page_number">
		<ul>
			...
			%>
			<li onclick="location.href='search.jsp?pageNum=<%=i%>&condition=<%=request.getParameter("condition")%>&search_input=<%=request.getParameter("search_input")%>'"><%=i%></li>
			<%
			}
			}
